...
// ✅ 健壯的 header row loader
async function safeLoadHeader(sheet) {
  try {
    await sheet.loadHeaderRow(1); // 嘗試設為第一列 (最常見)
  } catch (e) {
    console.warn('⚠️ loadHeaderRow(1) failed, try row 2');
    await sheet.loadHeaderRow(2); // fallback
  }
}
